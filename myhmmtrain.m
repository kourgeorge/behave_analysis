function [guessTR,guessE1,guessE2,logliks] = myhmmtrain(seqs,exptype,guessTR,guessE1,guessE2,varargin)
...
[checkE, numEmissions] = size(guessE1);
if checkE ~= numStates
...
E1 = zeros(numStates,numEmissions);
E2 = zeros(numStates,numEmissions);
...
for iteration = 1:maxiter
    ...
    for count = 1:numSeqs
        ...
        if baumwelch   % Baum-Welch training
            ...
            [~,logPseq,fs,bs,scale] = myhmmdecode(seq,exptype, guessTR,guessE1,guessE2);
            ...
            for k = 1:numStates
                for l = 1:numStates
                    for i = 1:seqLength
                        ...
                        TR(k,l) = TR(k,l) + exp( logf(k,i) + logGTR(k,l) + tmplogGE(l,seq(i+1)) + logb(l,i+1))./scale(i+1);
                    end
                end
            end
            for k = 1:numStates
                for i = 1:numEmissions
                    ...
                    E1(k,i) = E1(k,i) + sum(exp(logf(k,pos_homo)+logb(k,pos_homo)));
                    E2(k,i) = E2(k,i) + sum(exp(logf(k,pos_hetro)+logb(k,pos_hetro)));
                end
            end
...
%             E1 = E1 + iterE;
    ...
    totalEmissions1 = sum(E1,2);
    totalEmissions2 = sum(E2,2);
    totalTransitions = sum(TR,2);
    
    % avoid divide by zero warnings
    guessE1 = E1./(repmat(totalEmissions1,1,numEmissions));
    guessE2 = E2./(repmat(totalEmissions2,1,numEmissions));
    ...
    guessE1(isnan(guessE1)) = 0;
    guessE2(isnan(guessE2)) = 0;
    
    if verbose
        if iteration == 1
            fprintf('%s\n',getString(message('stats:hmmtrain:RelativeChanges')));
            fprintf('   Iteration       Log Lik    Transition     Emmission1    Emmission2\n');
        else 
            fprintf('  %6d      %12g  %12g  %12g  %12g\n', iteration, ...
                (abs(loglik-oldLL)./(1+abs(oldLL))), ...
                norm(guessTR - oldGuessTR,inf)./numStates, ...
                norm(guessE1 - oldGuessE1,inf)./numEmissions, ...
                norm(guessE2 - oldGuessE2,inf)./numEmissions);
        end
    end
    ...
    if (abs(loglik-oldLL)/(1+abs(oldLL))) < tol
        if norm(guessTR - oldGuessTR,inf)/numStates < trtol
            if (norm(guessE1 - oldGuessE1,inf)/numEmissions < etol) && (norm(guessE2 - oldGuessE2,inf)/numEmissions < etol)
                if verbose
